...
def fmt_desemb(tmib_n: int, m9_n: int, other: Dict[str, int]) -> str:
    parts: List[str] = []
    if tmib_n > 0:
        parts.append(f"TMIB:{tmib_n}")
    if m9_n > 0:
        parts.append(f"M9:{m9_n}")
    if other:
        for k, v in sorted(other.items(), key=lambda item: short_plat(norm_plat(item[0]))):
            if int(v) <= 0:
                continue
            parts.append(f"{short_plat(norm_plat(k))}:{int(v)}")
    if not parts:
        return "0"
    return ", ".join(parts)
...
def parse_stop_part(part: str) -> StopOps:
    ...
    for tok in rest_wo_braces.split():
        ...
        if re.match(r"^\+\d+$", t):
            ...
            explicit_drop = re.match(r"^-([^:\s]+)\s*:\s*(\d+)$", t)
            if explicit_drop:
                origin = explicit_drop.group(1).strip()
                n = int(explicit_drop.group(2))
                origin_short = short_plat(norm_plat(origin))
                if origin_short == "TMIB":
                    ops.disemb_tmib += n
                elif origin_short == "M9":
                    ops.disemb_m9 += n
                else:
                    ops.disemb_other[origin] = ops.disemb_other.get(origin, 0) + n
                continue
            raise ValueError(
                f"Token invalido na rota: '{t}'. Use apenas +N, -N ou -ORIGEM:N."
            )
...
def parse_route(route: str) -> List[StopOps]:
    ...
    stops = [parse_stop_part(p) for p in parts]

    for idx, stop in enumerate(stops):
        generic = int(stop.board_generic)
        if generic <= 0:
            continue
        origin_norm = norm_plat(stop.plat)
        inferred: Dict[str, int] = {}
        for future_stop in stops[idx + 1 :]:
            for origin, qty in future_stop.disemb_other.items():
                if norm_plat(origin) != origin_norm or int(qty) <= 0:
                    continue
                dest_short = short_plat(norm_plat(future_stop.plat))
                inferred[dest_short] = inferred.get(dest_short, 0) + int(qty)

        inferred_total = sum(inferred.values())
        if inferred_total != generic:
            raise ValueError(
                f"Embarque generico '+{generic}' em {stop.plat} nao fecha com os desembarques explicitos posteriores."
            )
        for destination, qty in inferred.items():
            stop.board_other[destination] = stop.board_other.get(destination, 0) + int(qty)
        stop.board_generic = 0
...
def create_input_template(path: str, speeds: Dict[str, float]):
    ...
    explanation = (
        "COMO DIGITAR A ROTA: +N = embarque na plataforma atual | -N = desembarque de pax vindos do TMIB | "
        "-ORIGEM:N = desembarque de pax vindos de ORIGEM\n"
        ...
        "Rota: TMIB +22/M10 -5/M9 -7 +4/M6 -4 +1/B2 -4 -M9:4/B1 -2 -M6:1"
    )
    explanation = (
        "COMO DIGITAR A ROTA: +N = embarque na plataforma atual | "
        "-N = desembarque de pax vindos do TMIB | "
        "-ORIGEM:N = desembarque de pax vindos de ORIGEM\n"
        "EXEMPLO: Lancha pega 22 no TMIB â†’ M10 deixa 5 â†’ M9 deixa 7 e pega 4 p/ B2 â†’ "
        "M6 deixa 4 e pega 1 p/ B1 â†’ B2 deixa 8 (4 TMIB + 4 M9) â†’ B1 deixa 3 (2 TMIB + 1 M6). "
        "Rota: TMIB +22/M10 -5/M9 -7 +4/M6 -4 +1/B2 -4 -M9:4/B1 -2 -M6:1"
    )
    ...
    instructions = [
        ...
        "SINTAXE DA ROTA:",
        "  - Separar paradas com '/'",
        "  - +N = embarcar N passageiros na plataforma atual",
        "  - -N = desembarcar N passageiros vindos do TMIB",
        "  - -ORIGEM:N = desembarcar N passageiros vindos de ORIGEM",
        "",
        "EXEMPLOS DE ROTAS:",
        "  TMIB +22/M9 -6 +6/M6 -2/B2 -14/B1 -M9:6",
        ...
        "",
        "  M9 +22/M6 +1 -M9:11/B1 -M9:5/TMIB -M9:6 -M6:1",
        "    -> Sai de M9 com 22 pax",
        "    -> Em M6: desembarca 11 M9, pega 1 pax com destino TMIB",
    ...
    example_data = [
        ["SURFER 1931", "06:20", "TMIB +16/M9 -16 +13/TMIB -M9:13 +20/M9 -6/M4 -4/M2 -5/M1 -5"],
        ...
        ["SURFER 1930", "05:20", "M9 +22/M6 +1 -M9:11/B1 -M9:5/TMIB -M9:6 -M6:1"],
    ]
