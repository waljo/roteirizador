...
def _parse_route_part(part: str) -> Optional[_RoutePart]:
    ...
    platform = _short_platform(tokens[0])
    remainder = tokens[1] if len(tokens) > 1 else ""
    generic_pickup = 0
    pickups_to: Dict[str, int] = defaultdict(int)
    deliveries: Dict[str, int] = defaultdict(int)

    def _consume_braced(match: re.Match[str]) -> str:
        nonlocal generic_pickup
        target = _short_platform(match.group(1))
        sign = match.group(2)
        qty = int(match.group(3))
        if sign == "+":
            pickups_to[target] += qty
        else:
            deliveries[target] += qty
        return " "

    remainder = _BRACED_TOKEN_RE.sub(_consume_braced, remainder)
    for match in _M9_DROP_RE.finditer(remainder):
        deliveries["M9"] += int(match.group(1))
    remainder = _M9_DROP_RE.sub(" ", remainder)
    for match in _SIMPLE_DROP_RE.finditer(remainder):
        deliveries["TMIB"] += int(match.group(1))
    remainder = _SIMPLE_DROP_RE.sub(" ", remainder)
    for match in _SIMPLE_PICKUP_RE.finditer(remainder):
        generic_pickup += int(match.group(1))
